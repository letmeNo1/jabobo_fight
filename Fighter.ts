...
export class Fighter {
  ...
  loadSprites() {
    ...
    // New path structure: /assets/characters/{characterId}/{action}.png
    ...
    console.log(`[Fighter] Starting asset load for: ${this.characterId}`);

    Object.entries(mappings).forEach(([key, suffix]) => {
      const img = new Image();
      const src = `/assets/characters/${this.characterId}/${suffix}.png`;
      
      img.onload = () => {
        console.log(`✅ [${this.characterId}] Loaded: ${src}`);
      };
      
      img.onerror = () => {
        console.warn(`❌ [${this.characterId}] Failed to load: ${src} (Will use fallback color)`);
      };
